...
# Optional OpenAI (for ChatGPT-based feedback)
try:
    import openai
except Exception:
    openai = None
...
# Safe even in weird environments without __file__
try:
...
def get_ai_backend_status():
    ...
    Check which AI backends are available:
      ...
      - Hugging Face (via HF_API_TOKEN)
    Returns a dict with simple booleans + selected OpenAI model (if any).
    """
    # --- OpenAI status ---
...
# ---------- Safe text for DOCX (strip invalid XML chars) ----------
...
def _safe_docx_text(value) -> str:
    """
    Ensure text is a clean Unicode string without illegal XML characters
    so python-docx / lxml do not crash.
    """
    if value is None:
        return ""
...
def export_student_word(submissions, student_name):
    ...
    for ex_id, sub in subs.items():
        ...
        doc.add_paragraph(
            _safe_docx_text(f"Time Spent: {sub.get('time_spent_sec', 0):.2f} sec")
        )
        doc.add_paragraph(
            _safe_docx_text(f"Characters (not keystrokes): {sub.get('keystrokes', 0)}")
        )
...
# ---------------- Optional AI generator for exercises (HF only) ----------------
def ai_generate_text(prompt):
    """
    Simple HF-based generator for creating source texts (used only for exercise generation).
    Safe to leave without token; returns None on error.
    """
    HF_TOKEN = os.getenv("HF_API_TOKEN", "")
...
def quick_linguistic_hints(source_text: str, student_text: str):
    ...
    try:
        ...
        # Brackets & quotes balance (compare each side separately)
...
# ---------------- Adaptive Feedback (metrics + hints) ----------------
...
def generate_ai_feedback(prompt: str):
    ...
    # --- Option 1: OpenAI new client style (if available) ---
    openai_key = os.getenv("OPENAI_API_KEY", "")
    if not openai_key:
        try:
            if hasattr(st, "secrets"):
    ...
    if openai and openai_key:
        try:
            # Try new-style client (openai>=1.0)
            ...
            # Fall back to HF if OpenAI call failed
            pass
...
def localisation_lab():
    ...
    # --- individual localisation exercises (same as you had) ---
    # For brevity: they stay unchanged from your previous long code,
    # just using save_loc_submission where appropriate.
    # (If you want, I can send those sections again, but functionally
    # they are identical to what you already had.)

    # For now, assume your exercise_1..7 and exercise_stickers functions
    # are here, calling save_loc_submission(...) as in your previous code.

    # Place your previous exercise_1, exercise_2, ..., exercise_stickers
    # definitions here (unchanged), then your router:
    ...
    # Call the appropriate exercise_* function here (as in your original code).
